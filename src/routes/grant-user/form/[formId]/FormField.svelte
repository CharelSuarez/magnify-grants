<script lang="ts">
    ...
    const toggleCheckboxOption = (e : Event & { currentTarget: EventTarget & HTMLInputElement; }, option: string) => {
...
<Card.Root class="w-full p-8 bg-secondary">
	<Card.Content class="w-full p-0 space-y-2">
		...
		{#if field.type == FieldType.ShortAnswer}
			<Form.Field form={filterForm} name={field.id} class="flex flex-col items-center">
				<Form.Control let:attrs>
					<Input bind:value={$formData[field.id]} {...attrs} 
                    type="text" name={field.id} placeholder={'Type a short answer here.'}
					/>
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
		{:else if field.type == FieldType.Paragraph}
			<Form.Field form={filterForm} name={field.id} class="flex flex-col items-center">
				<Form.Control let:attrs>
                    <Textarea bind:value={$formData[field.id]} {...attrs}
                    ...
                    <Form.Control let:attrs>
                        {#each field.options as option}
                            <div class="flex items-center gap-2 mt-2">
                                <RadioGroup.Item class="h-5 w-5" {...attrs} value={option}/>
		...
		{:else if field.type == FieldType.Checkbox}
			<Form.Field form={filterForm} name={field.id} class="flex gap-4 items-center h-10">
				<Form.Control let:attrs>
                    {#each field.options as option}
                        <div class="flex items-center gap-2 mt-2">
                            <input checked={isInitialCheckboxOption(field.id, option)} class="h-5 w-5" type="checkbox" 
                                {...attrs} name={field.id} on:input={(e) => toggleCheckboxOption(e, option)}/>
                            <Form.Label class="font-normal">{option}</Form.Label>
                        </div>
                    {/each}
				</Form.Control>
		...
		{:else if field.type == FieldType.Dropdown}
			<Form.Field form={filterForm} name={field.id}>
				<Form.Control let:attrs>
					<Select.Root selected={{value: $formData[field.id], label: $formData[field.id]}} 
                ...
                <Form.Field form={filterForm} name={field.id} class="flex gap-4 items-center h-10">
                    <Form.Control let:attrs>
                        {#each {length: 10} as _, i}
                            <div class="flex items-center gap-2 mt-2">
                                <RadioGroup.Item class="h-5 w-5" {...attrs} value={(i + 1).toString()} 
        ...
        {:else if field.type == FieldType.Date}
            <Form.Field form={filterForm} name={field.id} class="flex gap-4 items-center h-10">
                <Form.Control let:attrs>
                    <DatePicker bind:value={$formData[field.id]} {...attrs}></DatePicker>
                </Form.Control>
                <Form.FieldErrors />
            </Form.Field>
        {:else if field.type == FieldType.Time}
            <Form.Field form={filterForm} name={field.id} class="flex gap-4 items-center h-10">
                <Form.Control let:attrs>
                    <Input bind:value={$formData[field.id]} {...attrs} type="text"
