...
<div class="min-h-full flex-1 flex-row p-20 mt-10 flex items-start justify-center gap-10 box-border">
	<div class="hidden md:flex basis-1/4 w-full">
	</div>
	<div class="min-h-screen hidden basis-1/4 flex-grow-0 md:flex flex-col justify-end
		...
		<Button size='lg' class="w-full text-md gap-2" disabled={!grant} on:click={() => submitForm()}>
			{#if isLoading}
			...
			<span>{"Save Form"}</span>
			{#if lastSaved}
				<Badge class="text-md pt-1 pb-1">Saved at {lastSaved.getHours()}:{lastSaved.getMinutes()}</Badge>
			{/if}
		</Button>
		<a class="w-full" href={`/grant-user/grant/${!grant ? "" : toShort(grant.id)}`} target="_blank" 
			on:click={(e)=>e.preventDefault()}>
			<Button on:click={()=>{goto(`/grant-user/grant/${!grant ? "" : toShort(grant.id)}`)}} 
				size='lg' variant='secondary'  class="w-full text-md gap-2" disabled={!grant}>
	...
	<div class="hidden xl:flex basis-1/4 w-full">
	</div>
</div>
